package de.di.erpconnect.license.check;
...
public class ELOLicenseChecker extends BasicLicenseChecker {
    ...
    public void check(LicenseKey key, int rounds) throws LicenseException {
        ELOERPClient eloClient = new ELOERPClient(settings, logger);
        ...
        if (!eloVersion.getValue().equals("Office")) {
            ...
            try {
                ...
                while (serverInfo == null && count < 10000) {
                    try {
                        eloClient = new ELOERPClient(settings, logger);
                        ...
                        if (count == 10000) {
                            ...
                            count++;
                            try {
                                logger.error("ELOix is not available, ErpConnector thread sleep 10 seconds");
                                Thread.sleep(10000);
...
//      System.out.println("Serno: "+serno);
            ...
            if (!serno.contains(client.getValue())) { // 08-10-2019: equals() mit contains() ersetzt
